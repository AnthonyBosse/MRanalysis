function bin=bin1dr(x,d,X,asc,err)
...
[bin.n,bin.mean,bin.median,bin.var,bin.min,bin.max]=deal(nan(1,N));
...
for j=1:N % loop through N "columns"/along row ( x = j = - )
  ...
  if bin.n(j)~=0 
    ...
    %xx = robust_mean(d(index),1);
      bin.mean(j)=nanmean(d(index));
      bin.median(j)=nanmedian(d(index));       % (these are the CORE bin-mean-
